...
Фронт передаёт его отдельным полем опций прогона — `residents` (целое, `>= 0`),
а не вложенным `targets_overrides`. Приоритет целей для жилой зоны:

`targets_overrides.residential.residents` → `residents` → расчёт от площади блоков.

В чате то же число вытаскивает из реплики модель (поле `patch.residents`,
«рассели 12 тысяч человек»). Чтобы оно пережило перезагрузку чата, итоговые опции
прогона сохраняются в `metadata` ответного сообщения — из текста реплики их
восстановить надёжно нельзя.
